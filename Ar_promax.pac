...
if (typeof __PAC_CACHE === "undefined") {
  __PAC_CACHE = { dns: {}, proxyForHost: {} };
}
...
function inList(host, arr) {
  ...
  for (var i = 0; i < arr.length; i++) {
    if (shExpMatch(host, lc(arr[i]))) return true;
  }
  return false;
}
...
function isLocalTarget(host) {
  ...
  if (resolved) {
    if (isPrivateIPv4(resolved)) return true;
    if (sameSubnet24(resolved)) return true;
  }
...
function FindProxyForURL(url, host) {
  ...
  if (inList(host, CONNECTIVITY)) {
    __PAC_CACHE.proxyForHost[host] = CHAIN_LOBBY;
    return CHAIN_LOBBY;
  }

  if (inList(host, HEAVY)) {
    __PAC_CACHE.proxyForHost[host] = CHAIN_LOBBY;
    return CHAIN_LOBBY;
  }

  if (inList(host, JO_HOSTS)) {
    __PAC_CACHE.proxyForHost[host] = CHAIN_LOBBY;
    return CHAIN_LOBBY;
  }

  if (inList(host, LOBBY_HOSTS)) {
    var pick = chooseProxyForHost(host, [PROXY_LOBBY_PRIMARY, PROXY_LOBBY_BACKUP]);
  ...
  if (shExpMatch(host, "*.pubg*") || shExpMatch(host, "*tencent*") || shExpMatch(host, "*qcloud*") || shExpMatch(host, "*qq.com")) {
    var pickFallback = null;
    if (shExpMatch(host, "*.cdn*") || shExpMatch(host, "*.cloud*") || shExpMatch(host, "*.edge*")) {
      pickFallback = PROXY_MATCH_PRIMARY;
    } else {
      pickFallback = chooseProxyForHost(host, [PROXY_LOBBY_PRIMARY, PROXY_LOBBY_BACKUP]);
    }
