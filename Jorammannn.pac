var PROXIES_CFG = [
    { ip: "2a13:a5c7:25ff:7000", socksPorts: [20001, 20002, 1080, 8080, 443], httpPorts: [3128, 8080, 443] },
    { ip: "91.106.109.12", socksPorts: [20001, 20002, 1080, 8080, 443], httpPorts: [3128, 8080, 443] },
    { ip: "2a01:4f8:c17:2e3f::1", socksPorts: [20001, 20002, 1080, 8080, 443, 8000], httpPorts: [3128, 8080, 443] },
    { ip: "213.186.179.25", socksPorts: [80, 8000, 443], httpPorts: [80, 8000, 443] }
];
...
// إضافة نطاقات IP الخاصة بالمزودين المحليين في الأردن
const LOCAL_IP_RANGES = [
    { start: "192.168.0.0", end: "192.168.255.255" },
    { start: "10.0.0.0", end: "10.255.255.255" },
    { start: "172.16.0.0", end: "172.31.255.255" },
    { start: "185.220.0.0", end: "185.220.255.255" }
];
...
function proxyTokensFor(ip, socksPorts, httpPorts) {
    ...
    const socksTokens = socksPorts.flatMap(port => [`SOCKS5 ${host}:${port}`, `SOCKS ${host}:${port}`]);
...
function handleError(host, error) {
