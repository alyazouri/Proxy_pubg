...
var ENABLE_SOCKS   = false;  // يمكن تفعيله إن كان لديك SOCKS يعمل
var ENABLE_HTTP    = true;   // إبقِه true للـ HTTP(S)

// -------------------- PROXY POOL (Jordan) -----------------
// رتّب البروكسيات من الأفضل إلى الأقل تفضيلاً.
// أضفت 61.109.106.12 كاحتياط كما طلبت سابقاً.
var PROXIES = [
  { ip: "91.106.109.12",  http: [443,808],     socks: [1080] },
  { ip: "61.109.106.12",  http: [443,8080],    socks: [1080] },
  { ip: "2a13:a5c7:25ff:7000", http: [443,8080], socks: [1080] } // IPv6 (يُكتب بين أقواس)
];
...
var PUBG_DOMAINS = [
  ...
  // Google services غالباً مطلوبة لتشغيل الخدمات المرتبطة
  ...
  // شبكات اجتماعية (تسجيل الدخول/التحقق أحياناً)
  "*.x.com","*.twitter.com","api.x.com","api.twitter.com","abs.twimg.com","pbs.twimg.com","t.co"
];

// استثناءات إعلام/فيديو وتطبيقات مراسلة (يفضل إبقاؤها DIRECT لتحسين التجربة العامة)
var GLOBAL_EXCLUDES = [
...
function FindProxyForURL(url, host) {
  ...
  if (BLOCK_IR_TLD && isIR(host)) {
    return "DIRECT"; // أو "PROXY 0.0.0.0:0" لكن الأفضل DIRECT لتجنّب مشاكل
  }
  ...
  // 3) إن كان ضمن نطاقات PUBG/الخدمات المساندة -> مرّره عبر البروكسي الأردني
  ...
  if (isPlainIPv4(host)) {
    // إن أردت إجبار كل IP عبر البروكسي فعّل FORCE_ALL
